...
function buildFunctionParameters(items: any): Nethereum.Generators.Model.Parameter[] {
...
function buildEventParameters(items: any): Nethereum.Generators.Model.Parameter[] {
    var parameterOrder = 0;
    var parameters = [];
    for (var i = 0, len = items.length; i < len; i++) {
        parameterOrder = parameterOrder + 1;
...
function buildContract(abiStr: string): Nethereum.Generators.Model.ContractABI {
...
function generateAllFiles(abi: string, byteCode: string,
    contractName: string,
...
function generateProjectClassLibrary(projectName: string, basePath: string) {
